...
Cloud Run の `/tmp` はインスタンス単位のインメモリ FS なので、`/tmp/instance-id`（PR #20 までは `/tmp/fs-check-instance-id`） に初回アクセス時に乱数を置いておけば、同じインスタンスの間は同じ値、入れ替わると別の値になる。(e) ではこれでインスタンスの入れ替わりを確認する。
...
- **#8（タイムアウト・ステートレス性）**: (c) で `flock` が同一インスタンス内で直列化されることを確認した（後着 2.5 秒待ち）ので、二重送信対策は `concurrency=1` にしなくても `flock` で直列化できる → `docs/06` で `Updater` として実装。`POST /update` の実測は read 0.1 秒 / write 0.2 秒 / S3 PUT 1.0 秒（WIF 初回込み）で、タイムアウト 300 秒に対して余裕がある
